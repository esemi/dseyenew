...
--
-- Хост: localhost
-- Время создания: Янв 19 2013 г., 13:40
...
CREATE TABLE IF NOT EXISTS `alliances` (
  ...
  KEY `status` (`status`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 PACK_KEYS=0 AUTO_INCREMENT=1575 ;
...
CREATE TABLE IF NOT EXISTS `cron_logs` (
  ...
  `type` enum('up','day','onlineStatus','scavenger','dshelpRA','oldRanks','newRanks','csv') NOT NULL,
  `text` mediumtext NOT NULL,
  `date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `result` enum('none','success','warning','fail') NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=49324 ;
...
CREATE TABLE IF NOT EXISTS `players` (
  ...
  KEY `id_rase` (`id_rase`),
  KEY `status` (`status`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=96471 ;
...
DELIMITER //
CREATE TRIGGER `players_delts` BEFORE UPDATE ON `players`
 FOR EACH ROW BEGIN 
		IF OLD.`rank_old` != NEW.`rank_old` 
		THEN SET NEW.delta_rank_old = CAST(NEW.rank_old AS SIGNED) - CAST(OLD.rank_old AS SIGNED);
		END IF;

		IF OLD.`bo` != NEW.`bo` 
		THEN SET NEW.delta_bo = CAST(NEW.bo AS DECIMAL(11,2)) - CAST(OLD.bo AS DECIMAL(11,2));
		END IF;
	END
//
DELIMITER ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=573454 ;
...
CREATE TABLE IF NOT EXISTS `players_input` (
  ...
  KEY `id_world` (`id_world`),
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=317896 ;
...
CREATE TABLE IF NOT EXISTS `players_max_bo_delta` (
  ...
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=128332 ;
...
CREATE TABLE IF NOT EXISTS `players_max_rank_old_delta` (
  ...
  KEY `id_world` (`id_world`,`date`),
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=915390 ;
...
CREATE TABLE IF NOT EXISTS `players_output` (
  ...
  KEY `id_world` (`id_world`),
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=289479 ;
...
CREATE TABLE IF NOT EXISTS `players_trans_colony` (
  ...
  KEY `id_player` (`id_player`),
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=492841 ;
...
CREATE TABLE IF NOT EXISTS `players_trans_gate` (
  ...
  KEY `id_player` (`id_player`),
  KEY `date` (`date`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=28590 ;
...
CREATE TABLE IF NOT EXISTS `todo` (
  ...
  `status` enum('work','wait') NOT NULL DEFAULT 'wait',
  `title` varchar(200) NOT NULL,
  PRIMARY KEY (`id`)
